	...
	do {
		...
		if (strcmp(token, "[") == 0) {
			...
			if (type == EVENT_ITEM)
				...
				type = read_token(&token);
				if (type == EVENT_NONE) {
					do_warning_event(event, "%s", "failed to find token");
					goto fail;
				}
			}
		...
		*tok = token;
		...
		if ((strcmp(arg->op.op, "*") == 0) &&
			...
			if (left->type != PRINT_ATOM) {
				do_warning_event(event, "%s", "bad pointer type");
				goto out_free;
			}
...
eval_type_str(unsigned long long val, const char *type, int pointer)
{
	...
	if (pointer) {
		...
		if (type[len-1] != '*') {
			do_warning("%s", "pointer expected with non pointer type");
			return val;
		}
...
eval_type(unsigned long long val, struct print_arg *arg, int pointer)
{
	if (arg->type != PRINT_TYPE) {
		do_warning("%s", "expected type argument");
		return 0;
	}
...
process_paren(struct event_format *event, struct print_arg *arg, char **tok)
{
	...
	if (event_item_type(type) ||
		...
		if (arg->type != PRINT_ATOM) {
			do_warning_event(event, "%s", "previous needed to be PRINT_ATOM");
			goto out_free;
		}
		     ...
		     struct event_format *event, struct print_arg *arg)
{
	...
	for (i = 0; i < func_handle->nr_args; i++) {
		...
		default:
			 ...
			 */
			do_warning_event(event, "%s", "Unexpected end of arguments\n");
			goto out_free;
		}
		farg = farg->next;
		param = param->next;
...
static struct print_arg *make_bprint_args(char *fmt, void *data, int size, struct event_format *event)
{
	...
	if (!field) {
		...
		if (!field) {
			do_warning_event(event, "%s", "can't find buffer field for binary printk");
			return NULL;
		}
		ip_field = pevent_find_field(event, "ip");
		if (!ip_field) {
			do_warning_event(event, "%s", "can't find ip field for binary printk");
			return NULL;
		}
		  ...
		  struct event_format *event)
{
	...
	if (!field) {
		...
		if (!field) {
			do_warning_event(event, "%s", "can't find format field for binary printk");
			return NULL;
		}
...
static int is_printable_array(char *p, unsigned int len)
{
	...
	for (i = 0; i < len && p[i]; i++)
		if (!isprint((unsigned char)p[i]) && !isspace((unsigned char)p[i]))
		    return 0;
	return 1;
}
...
void pretty_print(struct trace_seq *s, void *data, int size, struct event_format *event)
{
	struct pevent *pevent = event->pevent;
 ...
 cont_process:
			...
			case '*':
				...
				if (!arg) {
					do_warning_event(event, "%s", "no argument match");
					event->flags |= EVENT_FL_FAILED;
					goto out_failed;
				}
			...
			case 'u':
				...
				/* should never happen */
				if (len > 31) {
					do_warning_event(event, "%s", "bad format!");
					event->flags |= EVENT_FL_FAILED;
					len = 31;
				}
			...
			case 's':
				if (!arg) {
					do_warning_event(event, "%s", "no matching argument");
					event->flags |= EVENT_FL_FAILED;
					goto out_failed;
				}
				   ...
				   char *name, ...)
{
	...
	if (!func_handle) {
		do_warning("%s", "Failed to allocate function handler");
		return PEVENT_ERRNO__MEM_ALLOC_FAILED;
	}
	...
	if (!func_handle->name) {
		do_warning("%s", "Failed to allocate function name");
		free(func_handle);
		return PEVENT_ERRNO__MEM_ALLOC_FAILED;
	}
	...
	for (;;) {
		...
		if (!param) {
			do_warning("%s", "Failed to allocate function param");
			ret = PEVENT_ERRNO__MEM_ALLOC_FAILED;
			goto out_free;
		}
		param->type = type;
				  ...
				  pevent_event_handler_func func, void *context)
{
 ...
 not_found:
	...
	if (!handle) {
		do_warning("%s", "Failed to allocate event handler");
		return PEVENT_ERRNO__MEM_ALLOC_FAILED;
	}
	...
	if ((event_name && !handle->event_name) ||
		...
		do_warning("%s", "Failed to allocate event/sys name");
		free((void *)handle->event_name);
		free((void *)handle->sys_name);
		free(handle);
		return PEVENT_ERRNO__MEM_ALLOC_FAILED;
